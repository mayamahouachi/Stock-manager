...
struct files Select(FILE *f,char doru){//retourne 2file qui sont remplis des donnees qu'on voulait selectionnes et un autre rempli des donnees a ne pas toucher
	int i,o,j,test;
    char w;
    char m='u';
    FILE *ft;
	...
	while(fscanf(f," %c %s %s %d %d:%d:%d %d %d",&op,&T.day,&T.month,&T.dym,&T.time.h,&T.time.m,&T.time.s,&T.year,&p.productID)!=EOF){
		...
		if (op=='a'){
			printf("You added");
				...
				if((toupper(op)=='U')||(toupper(op)=='X')){//
			printf("You updated");
									}
				else{ 
			if (toupper(op)=='D'){//fuck december
			printf("You deleted");}
					}
